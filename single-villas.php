...
$sleeps = (!empty($property_sleeps) ? 'Sleeps ' . $property_sleeps : '');
...
<div class="row property-title">
	<div class="large-12 columns">
		...
		if ($villa_pretty_name) {
			...
			echo '<h1 class="pretty-name">' . esc_html($villa_pretty_name) . '</h1>';
			the_title('<h2>', '</h2>');
...
<div class="row">
	<div class="small-12 large-8 columns">
				...
				<?php foreach ($images as $image) : ?>
					<div>
						...
						if (!empty($image_attributes[0])) :
							?>
							<img data-lazy="<?php echo esc_url($image_attributes[0]); ?>" alt="" />
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
		...
		<div id="property-map<?php echo ! $images ? ' force-display' : ''; ?>" style="display:none;">
			<?php get_template_part('templates/single-property-map'); ?>
		</div>

		<?php if ($villa_code = get_field('villa_manager_code')) : ?>
			<div style="display:none;" class="row villa-manager-code" id="property-availability">
		...
		<div class="toggle-buttons">
			...
			<?php endif; ?>
			<a class="button toggle-property-map <?php echo ! $images ? ' active' : ''; ?>" href="#">View Map</a>
			<?php if (get_field('villa_manager_code')) : ?>
				<a class="button toggle-property-availability" href="#">Show Availability</a>
			<?php endif; ?>
		</div>

		<?php
		$loc_t = get_the_terms($post->ID, 'property_location');
		$loc1 = (!empty($loc_t[0]) ? $loc_t[0]->name : '');
		$loc2 = (!empty($loc_t[1]) ? $loc_t[1]->name : '');
		...
		$property_type = 'villa';
		if ((int) $property_sleeps >= 12) {
			echo '<blockquote class="property-desc"><p>This ' . esc_html($locations) . ' ' . esc_html($property_type) . ' comfortably sleeps 12 guests.</p></blockquote>';
		} else {
			echo '<blockquote class="property-desc"><p>This ' . esc_html($locations) . ' ' . esc_html($property_type) . ' comfortably sleeps ' . esc_html((string) $property_sleeps) . ' guests.</p></blockquote>';
		}
		?>
		...
		<div class="hide-for-large-up">
			...
			if ((int) $property_sleeps >= 12) {
				...
				echo '<div class="single-property-sleeps">';
				echo '<h3>This Villa sleeps ' . esc_html((string) $property_sleeps) . '</h3>';
				echo '</div>';
			}
			?>
		...
		<h2 style="padding-top:20px;"><?php esc_html_e('About', 'ibiza-villas-2000'); ?> <?php echo esc_html((string) get_field('villa_pretty_name')); ?></h2>
		<?php the_content(); ?>
	...
	<div class="sticky-price-container">
		...
		<span class="sticky-price-amount">
			...
			if ($price_from_euros) {
				...
				echo '€' . number_format($price_from_euros_day);
			} else {
