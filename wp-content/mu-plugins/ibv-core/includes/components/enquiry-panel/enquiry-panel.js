...
( function () {
	...
	function bindEnquiry( panel, form ) {
		...
		var EUR_WHOLE = new Intl.NumberFormat( 'en-GB', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 } );
		...
		function showDatedOverviewPrice( stayTotal, nights ) {
			...
			ovAmount.textContent = EUR_WHOLE.format( Math.round( stayTotal ) );
	...
	// cannot skip us. boot() is idempotent (per-form __ibvVilla* guards).
	if ( window.jQuery ) {
