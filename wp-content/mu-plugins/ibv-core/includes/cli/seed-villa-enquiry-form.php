 ...
 * are no real entries pre-launch. Stores the id in `ibv_villa_enquiry_form_id`.
 *
 * Run (Local Site Shell):  wp eval-file wp-content/mu-plugins/ibv-core/includes/cli/seed-villa-enquiry-form.php
 *
...
if ( $existing_id ) {
	...
	if ( is_wp_error( $result ) ) {
		echo 'Error updating Villa Enquiry form: ' . $result->get_error_message() . "\n";
		return;
	}
	update_option( 'ibv_villa_enquiry_form_id', $existing_id );
	echo 'Updated Villa Enquiry form id: ' . $existing_id . "\n";
	return;
}
...
if ( is_wp_error( $result ) ) {
	echo 'Error creating Villa Enquiry form: ' . $result->get_error_message() . "\n";
	return;
}

update_option( 'ibv_villa_enquiry_form_id', (int) $result );
echo 'Created Villa Enquiry form id: ' . (int) $result . "\n";
